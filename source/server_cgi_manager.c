...
/*
cgi_execute_script(HTTP_SESSION *http_session, const char *filename)
...
void cgi_execute_script(HTTP_SESSION *http_session, const char *filename)
{
	...
	int valid_res = -1;			/* Przechowuje rezultat dzia�ania funkcji http_valid_cgi_file */
	long hdr_len = 0;			/* Rozmiar wczytanych nag��wk�w */
	int i = 0, j = 0;			/* Zmienne pomocnicze dla p�tli */

	/* Alokacja pami�ci na obiekty, na kt�rych b�dzie grzeba� funkcja http_valid_cgi_file */
	...
	(patrz definicja funkcji http_valid_cgi_file()) */
	http_valid_cgi_file(filename, &valid_res, exec_filename, cgi_param);
	...
	if(valid_res == 1)	/* Plik jest wykonywalny sam z siebie */
		strncpy(cgi_script_exec, filename, MAX_PATH_LENGTH);

	else if(valid_res == 2) /* Plik wymaga zewn�trznej aplikacji */
	{
	...
	else if(valid_res == 3) /* Plik wymaga zewn�trznej aplikacji z parametrem */
	{
		strncpy(cgi_script_exec, exec_filename, MAX_PATH_LENGTH);
		strncat(cgi_script_exec, " ", MAX_PATH_LENGTH);
		strncat(cgi_script_exec, filename, MAX_PATH_LENGTH);
	...
	http_valid_cgi_file. Odczyt danych w trybie binarnym. */
	...
	if(!cgi_script_file)
	{
		...
		/* Problem - plik zosta� wcze�niej odnaleziony w http_process_session, ale teraz si� go nie da otworzy� */
	...
	strncpy(add_hdr, http_get_message_header(buf, result_size), BIG_BUFF_SIZE);

	hdr_len = strlen(add_hdr);

	/* Wczytanie contentu */
	/* U�yty modyfikator 2 (poni�ej i w http_send_header) = "\r\n" = pusta linia mi�dzy nag��wkami a contentem */
	...
	if(strlen((char*)cgi_result) == 0)
		...
		http_send_header(http_session, HTTP_200_OK, NULL, result_size-hdr_len-2, (char*)cgi_body, add_hdr);
...
static void cgi_set_env_variables(HTTP_SESSION *http_session, const char *filename)
{
	...
	else /* Zerowanie zmiennych systemowych */
	{
...
/*
cgi_load_script_configuration(void)*/
int cgi_load_script_configuration(const char *filename)
{
	FILE *cfg_file;
	...
	print_log("Loading CGI script configuration...");
		...
		if((sscanf(buf, "%s %s %s", ext, exec_name, param) == 3) && (cgi_other_count < 8))
		{
			...
			if(strncmp(exec_name, STD_EXEC, STD_BUFF_SIZE) != 0)
			{
				...
				if(file_stat.st_mode & S_IXOTH)
					print_log("\t- %d: \"%s\" assigned to \"%s\" with param \"%s\".\n", cgi_other_count, ext, exec_name, param);
				else
				{
					print_log("%s is not executable.\n", exec_name);
					continue; /* Nie jest... */
				}

			}
			else print_log("\t- %d: \"%s\" assigned to \"%s\" with param \"%s\".\n", cgi_other_count, ext, exec_name, param);
