...
/*
http_get_message_body(const char *content_data)
...
{
	char *p1 = (char*)malloc(MAX_BUFFER_CHAR);
...
/*
http_get_message_header(const char *content_data, long content_length)
...
char* http_get_message_header(const char *content_data, long content_length)
{
	...
	for(i = 0; i < content_length; ++i)
	{
		...
		else /* Napotkano koniec linii */
		{
			if(strlen(hdr_line) == 1) /* Je�eli d�ugo�� linii = 1 to jest to pusta linia - po niej nast�puje content */
				break;
...
/*
http_get_query_string(HTTP_SESSION *http_session)
@http_session - wska�nik do pod��czonego klienta
- zwraca:
...
char* http_get_query_string(HTTP_SESSION *http_session)
{
	...
	/* Metoda POST - query string to message body */
		...
		result = http_get_message_body(http_session);
...
/*
http_get_mime_type(const char *filename)
...
/*
http_get_header_value(const char *header, const char *requested_value_name)
...
char* http_get_header_value(const char *header, const char *requested_value_name)
{
	...
	if(!tmp_header_val) /* Brak ��danego nag��wka - zwracamy standardowy text/html */
		return "";

	/* Alokacja pami�ci */
	result_handler = (char*)malloc(BIG_BUFF_SIZE);
	...
	while((dst[count] = result_handler[count]) != '\015')
		if(result_handler[count] != '\015') count++;
	dst[count] = '\0';
...
/*
http_send_header(HTTP_SESSION *http_session, const char *http_status_code, const char *http_mime_type, size_t http_content_length, char *time_last_modified, const char *content_data, const char* add_headers)
...
void http_send_header(HTTP_SESSION *http_session, const char *http_status_code, const char *http_mime_type, size_t http_content_length, const char *content_data, const char* add_headers)
{
	...
	/* Alokacja pami�ci */
	http_header_to_send = (char*)malloc(MAX_BUFFER_CHAR+1);
	mem_allocated(http_header_to_send, 9);
	http_single_header_line = (char*)malloc(STD_BUFF_SIZE_CHAR);
...
/*
http_send_error(HTTP_SESSION *http_session, const char *http_status_code, const char *http_error_message, const char* add_headers)
@http_session - wska�nik do pod��czonego klienta
...
void http_send_error(HTTP_SESSION *http_session, const char *http_status_code, const char *http_error_message, const char* add_headers)
{
	...
	/* Wysy�ka nag��wka HTTP */
	data_send_http_response(http_session, http_header_to_send, strlen(http_header_to_send));

	/* Wysy�ka tre�ci b��du, je�eli istnieje */
	if(http_error_message)
		data_send_http_response(http_session, http_error_message, len);
...
/*
http_send_file(HTTP_SESSION *http_session, const char *filename)
@http_session - wska�nik do pod��czonego klienta
...
void http_send_file(HTTP_SESSION *http_session, const char *filename)
{
	...
	file_params w nadrz�dnej funkcji http_process_session */	
	file = fopen(filename, READ_BINARY);

	/* Nie uda�o si� otworzy� pliku, cho� istnieje - problem z serwerem? */
	if(!file)
		http_send_error(http_session, HTTP_500_SERVER_ERROR, HTTP_ERR_500_MSG, NULL);
	else
	{
		...
		if(http_session->http_info.date_if_modified_since)
		{
			...
			{
				fclose(file);
				http_send_header(http_session, HTTP_304_NOT_MODIFIED, HEADER_STD_CONTENT_TYPE, 0, NULL, NULL);
				return;
			}
		}
		...
		if(http_session->http_info.date_if_unmodified_since)
		{
			...
			{
				fclose(file);
				http_send_header(http_session, HTTP_412_PRECONDITION_FAILED, HEADER_STD_CONTENT_TYPE, 0, NULL, NULL);
				return;
			}
		}
		...
		if(filesize <= 0)
			http_send_error(http_session, HTTP_204_NO_CONTENT, HTTP_ERR_204_MSG, NULL);
		else
		{
			...
			if((http_session->http_info.range_st < 0)&&(http_session->http_info.range_en < 0))
			{
				...
				http_send_header(http_session, HTTP_200_OK, http_get_mime_type(filename), filesize, NULL, NULL);
				...
				{
					if(data_send_http_response(http_session, buf, len) == 0)
			...
			else /* Wysy�ka wybranego fragmentu pliku */
			{
				...
				if((fseek(file, http_session->http_info.range_st, SEEK_SET) != 0) || (total <= 0))
					http_send_error(http_session, HTTP_416_REQUESTED_RANGE_NOT_SATISFIABLE, HTTP_ERR_416_MSG, NULL);
				else
				{
					/* Dodanie nag��wka "Content-Range" */
					add_hdr = (char*)malloc(STD_BUFF_SIZE_CHAR);
					...
					http_send_header(http_session, HTTP_206_PARTIAL_CONTENT, http_get_mime_type(filename), total+1, NULL, add_hdr);
					free(add_hdr);
					add_hdr = NULL;
					...
					if(fread(buf, sizeof(char), total, file) > 0)
						data_send_http_response(http_session, buf, total+1);
					else /* Napotkano b��d = 500 */
						http_send_error(http_session, HTTP_500_SERVER_ERROR, HTTP_ERR_500_MSG, NULL);
...
/*
http_process_session(HTTP_SESSION *http_session)
@http_session - wska�nik do pod��czonego klienta
...
void http_process_session(HTTP_SESSION *http_session)
{
	...
	if(len_loc > 1)
	{
		...
		if((strstr(http_session->http_info.http_local_path, "?") != NULL) || (http_session->http_info.method_name == POST))
		{
			...
			strncat(local_file_path, http_get_cgi_name(http_session), MAX_PATH_LENGTH);
			...
			strncpy(http_session->http_info.query_string, http_get_query_string(http_session), MAX_PATH_LENGTH);
		...
		else
		{
			...
			http_valid_cgi_file(local_file_path, &cgi_valid_res, NULL, NULL);
			if(cgi_valid_res > 0)
				http_session->http_info.is_cgi = 1;
			else http_session->http_info.is_cgi = 0;
		}
	} /* Jeste�my w g��wnym katalogu */
	else strncpy(local_file_path, app_path, MAX_PATH_LENGTH);
	...
	if(strncmp(file_get_name(local_file_path), "", 1) == 0)
		strncat(local_file_path, http_get_index(local_file_path), MAX_PATH_LENGTH);

	/* Alokacja pami�ci */
	ht_access_pwd = (char*)malloc(STD_BUFF_SIZE_CHAR);
	...
	{
		file_ext = (char*)malloc(MICRO_BUFF_SIZE_CHAR);
		...
		if(strncmp(file_ext, "", MICRO_BUFF_SIZE) == 0)
		{
			...
			if(local_file_path[strlen(local_file_path)-1] != C_SLASH)
			{
				...
				{
					/* Stworzenie dodatkowej informacji do nag��wna wysy�anego przez http_send_error z prawid�ow� �cie�k� */
					add_hdr = (char*)malloc(MAX_PATH_LENGTH_CHAR);
					sprintf(add_hdr, "%s%s/\r\n", HEADER_LOCATION, http_session->http_info.http_local_path);
					http_send_error(http_session, HTTP_302_FOUND, HTTP_ERR_302_MSG, add_hdr);
					free(add_hdr);
					add_hdr = NULL;
				}
				else http_send_error(http_session, HTTP_400_BAD_REQUEST, HTTP_ERR_400_MSG, NULL);
			} /*...plik nie istnieje, a index.html brak... */
			else http_send_error(http_session, HTTP_404_NOT_FOUND, HTTP_ERR_404_MSG, NULL);
		}
		else http_send_error(http_session, HTTP_404_NOT_FOUND, HTTP_ERR_404_MSG, NULL);
	...
	else /* Plik istnieje, ale jakie ma w�a�ciwo�ci ? */
	{
		if(file_params_val == 1) /* Plik istnieje i jest do odczytu */
		{
			/* Je�eli jest to skrypt CGI, to zostaje wykonany */
			if(http_session->http_info.is_cgi == 1)	/* Wcze�niej oznaczony jako CGI */
				cgi_execute_script(http_session, local_file_path);
			else /* Zwyk�e ��danie zasobu */
			{
				/*Zapytanie zweryfikowane - wysy�ka zawarto�ci zasobu */
				if(http_session->http_info.method_name != POST)
					http_send_file(http_session, local_file_path); /* Zapytanie HEAD jest weryfikowane w funkcji http_send_header */
			}
		}
		else if(file_params_val == 2) /* Plik istnieje, ale dost�p jest zabroniony */
			http_send_error(http_session, HTTP_403_FORBIDDEN, HTTP_ERR_403_MSG, NULL);
			...
			if(http_session->http_info.authorization)
			{
				if(strncmp(ht_access_pwd, http_session->http_info.authorization, STD_BUFF_SIZE) == 0)
				{
					...
					else /* Zwyk�e ��danie zasobu */
					{
						...
						if(http_session->http_info.method_name != POST)
							http_send_file(http_session, local_file_path); /* Zapytanie HEAD jest weryfikowane w funkcji http_send_header */
						else http_send_error(http_session, HTTP_401_AUTHORIZATION_REQUIRED, HTTP_ERR_401_MSG, HEADER_AUTHENTICATION);
					}
				} else http_send_error(http_session, HTTP_401_AUTHORIZATION_REQUIRED, HTTP_ERR_401_MSG, HEADER_AUTHENTICATION);
			}
			else http_send_error(http_session, HTTP_401_AUTHORIZATION_REQUIRED, HTTP_ERR_401_MSG, HEADER_AUTHENTICATION);			
...
/*
http_get_cgi_name(const char* http_local_path)
...
char* http_get_cgi_name(HTTP_SESSION *http_session)
{
	...
	else return http_session->http_info.http_local_path;
}

/*
http_get_range(HTTP_SESSION *http_session, int type)
@http_session - wska�nik do pod��czonego klienta
...
long http_get_range(HTTP_SESSION *http_session, int type)
{
	...
	ptr = (char*)malloc(STD_BUFF_SIZE_CHAR);
	strncpy(ptr, http_get_header_value(http_session->http_info.header, HEADER_RANGE), STD_BUFF_SIZE);
	...
	/* Rezerwacja pami�ci */
	range = (char*)malloc(SMALL_BUFF_SIZE_CHAR);
	mem_allocated(range, 7);

	temp_r = (char*)malloc(SMALL_BUFF_SIZE_CHAR);
...
/*
http_valid_cgi_file(const char *filename)
...
/*
http_get_index(const char *path)
